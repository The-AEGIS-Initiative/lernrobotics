...
function EditorSection({level, width}) { // creating the Editor class
  ...
  useEffect(()=>{
    ...
    if(appContext.isAuth){
      getData(`${process.env.REACT_APP_BACKEND_URL}/user/code/${level}`,
                    (res)=>{
                      const jsonData = res.json().then(
  ...
  }, [appContext.isAuth]); // Re-render on authentication status change

  // Capture Ctrl+S from editor to prevent annoying pop-ups
  useEffect(()=>{
    if(editorRef.current){ // Prevent null referene errors
